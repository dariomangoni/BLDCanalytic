...
for alfa_ind = 1:length(alfa)
    ...
    lambda_tilde_array(alfa_ind,:) = [alfa(alfa_ind), r, lambda_tilde];
end

figure('Name','Slot effects','NumberTitle','off');
plot([-flipud(lambda_tilde_array(2:end,1)*180/pi);lambda_tilde_array(:,1)*180/pi],[flipud(lambda_tilde_array(2:end,3));lambda_tilde_array(:,3)],'-','MarkerSize',6);
hold on;

%% calc harmonic component
lambda_mi_tilde = (1-1.6*beta*b0/tau_t)/Kc;
alfa_sa = 0;
alfa = 0:2*pi/2000:2*pi;
lambda_tilde_array_fourier = [alfa',ones(length(alfa),1).*r,ones(length(alfa),1).*lambda_mi_tilde];
for n=1:250
    for alfa_ind = 1:length(alfa)
        
        lambda_mi_tilde = -beta*4/pi/n*( 0.5+(n*b0/tau_t)^2/(0.78125-2*(n*b0/tau_t)^2))*sin(1.6*pi*n*b0/tau_t);

        lambda_tilde_array_fourier(alfa_ind,3) = lambda_tilde_array_fourier(alfa_ind,3) + (-1)^n*lambda_mi_tilde*cos(n*Qs*(alfa(alfa_ind)-alfa_sa));
    end
end

plot(lambda_tilde_array_fourier(:,1)/pi*180,lambda_tilde_array_fourier(:,3),'-','Color','r');
legend('Permeance','Permeance Fourier','Location','East')
